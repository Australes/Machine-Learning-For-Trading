...
def optimize_allocations(sd, ed, symbols, gen_plot):
    ...
    cons = {'type': 'ineq','fun' : lambda x: np.array(1-np.sum(np.array(x))),'jac' : lambda x: np.array([-1.0,-1.0,-1.0])}
    max_result = spo.minimize(find_portfolio_statistics, initial_guess, args=(df, gen_plot,),method='SLSQP',bounds=tuple(l), constraints = cons)
...
print (optimize_allocations('2016-04-26','2017-03-19',['GOOG','AAPL'], True))
